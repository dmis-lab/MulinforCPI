...
class Trainer():
    ...
    def train(self, train_loader: DataLoader, val_loader: DataLoader):
        ...
        for epoch in range(self.start_epoch, self.args.num_epochs + 1):  # loop over the dataset multiple times
            ...
            with torch.no_grad():
                metrics_val = self.predict(val_loader, epoch)
                print(metrics_val)
                metrics = metrics_val
                val_score = metrics[self.main_metric]
                wandb.log({'val_mse':val_score})
        ...
        self.model.load_state_dict(checkpoint['model_state_dict'])
        return self.evaluation(best_epoch, val_loader, data_split='val_best_checkpoint')

    def forward_pass(self, batch):
        targets = batch[-1]  # the last entry of the batch tuple is always the targets
        predictions = self.model(*batch[0],batch[1],batch[2],torch.LongTensor(batch[3].cpu().detach().numpy()))  # foward the rest of the batch to the model
        return self.loss_func(predictions, targets), predictions, targets

    def process_batch(self, batch, optim = None):
    ...
    def evaluation(self, best_epoch, data_loader: DataLoader, data_split: str = ''):
        self.model.eval()
        metrics = self.predict(data_loader, epoch=best_epoch)

        with open(os.path.join(self.writer.log_dir, 'evaluation_' + data_split +'{}'.format(best_epoch) + '.txt'), 'w') as file:
