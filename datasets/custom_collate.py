...
def dti_graph_collate(batch: List[Tuple]):
    graphs, morgan_fingers, prot_structures, protein_dist, targets = map(list, zip(*batch))
    # import pdb
    # pdb.set_trace()
    batched_graph = dgl.batch(graphs)
    targets = torch.stack(targets).float()
    if len(targets.shape) == 1:
        targets = targets.unsqueeze(-1)
    return [batched_graph], torch.stack(prot_structures).float(),torch.stack(protein_dist).float(), torch.stack(morgan_fingers).float(), targets
