...
class CusDataset(Dataset):
    ...
    def __init__(self, smiles_txt_path, device='cuda:0', transform=None, **kwargs):
        ...
        smiles_list = list(pd.read_csv(smiles_txt_path)['smiles'])
        self.labels = torch.tensor(list(pd.read_csv(smiles_txt_path)['HIV_active']))
        self.device = device
        # print(smiles_list)
        atom_slices = [0]
        edge_slices = [0]
    ...
    def __getitem__(self, idx):
        ...
        return tuple([g.to(self.device),self.labels[idx].to(self.device)])
