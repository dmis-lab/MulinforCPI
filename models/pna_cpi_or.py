...
class PNAcpi1(nn.Module):
    ...
    def __init__(self,
        ...
        self.protein_net1 = nn.Sequential(*[nn.Linear(65,20), nn.ReLU(), nn.Dropout(0.1)])
        # self.dist_net = nn.Conv2d(1,4,3)
        ...
        # self.conv_in_morgan = nn.Sequential(*[nn.Conv1d(in_channels=2048, out_channels=200, kernel_size=1),
        #     nn.BatchNorm1d(200),nn.ReLU()])

        self.conv_in = nn.Sequential(*[nn.Conv1d(in_channels=4000, out_channels=500, kernel_size=1),
            nn.BatchNorm1d(500),nn.ReLU()])
        self.convs = nn.ModuleList([nn.Conv1d(65, 65*2, 7, padding=7//2) for _ in range(3)]) 
        self.norm = nn.LayerNorm(500)
        # self.morgan_net = nn.Sequential(*[nn.Linear(2048,1024), nn.ReLU(), nn.Dropout(0.1),nn.Linear(1024,128), nn.ReLU(), nn.Dropout(0.1)])
        # MLP(in_dim=2048, hidden_size=readout_hidden_dim,
        #                   mid_batch_norm=readout_batchnorm, out_dim = 128,
        #                   layers=readout_layers, batch_norm_momentum=batch_norm_momentum, last_activation = last_activation_readout)
        ...
        self.multihead_att_inter = MultiheadAttentioninter(input_dim = 4, embed_dim = 4, num_heads = 1)

        self.out_prot_dist = nn.Sequential(*[nn.Linear(100,32), nn.ReLU(), nn.Dropout(0.1)])
    ...
    def forward(self, graph: dgl.DGLGraph, protein_feature: torch.Tensor, protein_dist_matrix: torch.Tensor, morgan_fingers: torch.Tensor):
        ...
        # morgan_out = self.conv_in_morgan(morgan_out_emb) # batch 200 4
        comp_re = self.multihead_att_comp(morgan_out,graph_feat,graph_feat) # batch 2048 4
        # comp_batch_emb_out = self.output(torch.flatten(comp_re, start_dim = 1)) # batch 128
        ...
        # prot_out = prot_batch_emb_out.view(prot_batch_emb_out.size(0),100,4)

        interaction = self.multihead_att_inter(comp_re,prot_batch_att,prot_batch_att)
